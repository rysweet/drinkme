...
| `core/ide` | corrupt project-load IO failure delegation, backup recovery policy seams, backup-directory path handling, VR project-loader save-path behavior, player export artifact shape, and editor save-copy archive roundtrip |
...
- The synthetic project/resource IO tests are intentionally minimal. They do not cover real StageIDE-generated projects, gallery resource subclasses, or historical fixtures. `ProjectFileUtilities` now has player export and editor save-copy smokes for synthetic projects and thumbnails, but not a full StageIDE export/save UI journey.

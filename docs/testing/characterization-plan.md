...
| `core/story-api-migration` | migration table ordering, applicability thresholds, representative text rewrite chains, synthetic project IO round-trip, synthetic resource IO round-trip, player/export JSON image/audio resource reads, JSON archive future-version detection, JSON player resource entry safety, corrupt manifest dispatch, explicit manifest decode error behavior, JSON resource identity isolation, XML resource identity isolation, model/generated type manifest-reference behavior, JSON `.a3c` resource-only type reads, JSON `.a3c` version behavior, and `JsonModelIo` export format selection |
| `core/ide` | corrupt project-load IO failure delegation, backup recovery policy seams, backup-directory path handling, URI project-loader backup/default-backup path classification, VR project-loader save-path behavior, player export artifact shape/resource readability, editor save-copy archive roundtrip, project-load success/failure dispatch decision planning, and default-backup copy behavior |
...
- NetBeans Alice library classpath: `Alice3Library.xml` is parsed as packaged XML, verifies required Alice/OpenJFX entries, rejects duplicate entries, and now uses `story-api.jar` without a trailing slash.
- Project load failure dispatch planning: `ProjectLoadFailureDispatchPlan` characterizes the post-dialog decision to load a backup, load the main project, show a new-project operation, or do nothing.
- Manifest decode errors: legacy `ManifestEncoderDecoder.fromJson()` still logs and returns `null`, while opt-in `fromJsonOrThrow()` surfaces malformed JSON for IO dispatch paths that must reject corrupt manifests.
- JSON `.a3c` version behavior: type archive readers now match player readers for future, missing, and corrupt `version.txt` behavior.
- XML resource identity isolation: XML project/type reads create fresh resource instances by UUID and rebind AST `ResourceExpression`s to the decoded project resources, avoiding static UUID-map mutation across archive reads.
- `JsonModelIo` export format seams: asset-free tests cover `.dae`/Collada, `.glb`/glTF, and `.a3c`/Alice format selection.
- URI project-loader path classification: tests cover normal project files, named backup directories, default backup directories, and new-project loader semantics.
- Default-backup copy behavior: tests cover missing and empty `.defaultbak` directories as no-ops, while real backup copy failures now propagate through the existing save `IOException` path.
...
- Default-backup copy is now directly covered at the `ProjectFileUtilities` seam for populated, missing, and empty `.defaultbak` directories; full StageIDE save UI coverage is still missing.

...
| `core/story-api-migration` | migration table ordering, applicability thresholds, representative text rewrite chains, synthetic project IO round-trip, synthetic resource IO round-trip, player/export JSON image/audio resource reads, JSON archive future-version detection, and JSON player resource entry safety |
| `core/ide` | corrupt project-load IO failure delegation, backup recovery policy seams, backup-directory path handling, VR project-loader save-path behavior, player export artifact shape/resource readability, and editor save-copy archive roundtrip |
...
- Project player export future-version detection: `JsonProjectIo.checkForFutureVersion()` now reads `version.txt` and reports future versions for manifest-declared `.a3w` archives, matching the XML reader behavior used by project loaders.

...
| First-lesson action chain | 15 | 2 | 8 | 5 | Only launch and window focus are fully done. RabbitHole can partially prove several backend steps, Run-frame creation, and desktop VM statement execution; rendering, save-menu completion, grading, and full lesson completion are still open. |
...
| 11 | Prove desktop Run VM statement execution | Not proven | Proven with VM listener artifact and runtime log after toolbar Run-frame proof | Partial: RabbitHole desktop VM statement execution is proven; original Alice, rendering, and completion are not |
...
| Build eatme into a comparison test runner | eatme now has target setup, comparison reports, scorecards, path checks, lesson-session checks, readiness checks, a first-lesson readiness sequence, Alice window activation, a machine-readable place-object no-go probe, proof hooks for object placement, procedure edit, bounded run-world, and project save, Ctrl+S desktop input dispatch, Ctrl+F5 Run shortcut dispatch, stale display cleanup, explicit test-only license preference seeding, a Run-window proof path, and desktop Run VM statement execution evidence. [RabbitHole PR #148](https://github.com/rysweet/RabbitHole/pull/148) writes an opt-in `run-window-created.json` file when Alice prepares the desktop Run frame. [RabbitHole PR #149](https://github.com/rysweet/RabbitHole/pull/149) writes bounded desktop Run VM listener evidence. [eatme PR #83](https://github.com/rysweet/eatme/pull/83) reads the Run-frame file, checks the fixed 1000x740 Alice launch geometry before clicking the toolbar Run button, and captures a screenshot. [eatme PR #84](https://github.com/rysweet/eatme/pull/84) reads the desktop execution file and runtime log. | eatme can compare startup behavior, check first-lesson evidence, find and focus the Alice main window under bare Xvfb, dispatch real desktop save and Run shortcuts as input evidence, record the license agreement as a blocker unless an explicit test switch seeds the isolated preferences, clean stale X display files left by crashed runs, ask RabbitHole to prove backend first-lesson actions, prove that a geometry-checked toolbar click opens the RabbitHole Run frame, and prove that RabbitHole desktop Run reached VM statement execution. Ctrl+F5 still does not open the Run window in the current Xvfb run. eatme still does not prove visible rendering, desktop save-menu completion, grading, or a full teacher/student lesson. |
...
| Desktop Run frame | eatme must know whether Alice actually opened the Run window and whether RabbitHole reached VM statement execution, not just whether a keypress was sent. | RabbitHole now has an opt-in `run-window-created.json` proof file when the Run frame is prepared and an opt-in `desktop-run-execution.json` proof file when the desktop Run path reaches VM statement execution. eatme now checks Alice's 1000x740 launch geometry, clicks the visible toolbar Run button, reads both proof files, checks the runtime log, and stores a screenshot. Ctrl+F5 still does not open the Run window in the latest real run. | [RabbitHole Run-window hook #148](https://github.com/rysweet/RabbitHole/pull/148), [RabbitHole desktop Run hook #149](https://github.com/rysweet/RabbitHole/pull/149), [eatme Run-window consumer #83](https://github.com/rysweet/eatme/pull/83), [eatme desktop Run consumer #84](https://github.com/rysweet/eatme/pull/84), [Run-window toolbar proof journal](docs/atlas/journal/0084-run-window-toolbar-proof.md), [desktop Run execution journal](docs/atlas/journal/0085-desktop-run-execution-evidence.md) | Replace the coordinate click with a stronger UI affordance or accessibility path; prove visible rendering and world completion after the Run frame opens. |
...
| [Run-window toolbar proof](docs/atlas/journal/0084-run-window-toolbar-proof.md) | Notes for RabbitHole's Run-window proof file and eatme's geometry-checked toolbar Run-frame proof. |
| [Desktop Run execution evidence](docs/atlas/journal/0085-desktop-run-execution-evidence.md) | Notes for RabbitHole's desktop Run VM listener evidence and eatme's matching execution artifact checks. |

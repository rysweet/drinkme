...
| Desktop Run frame | eatme must know whether Alice actually opened the Run window, not just whether a keypress was sent. | RabbitHole now has an opt-in `run-window-created.json` proof file when the Run frame is prepared. eatme now checks Alice's 1000x740 launch geometry, clicks the visible toolbar Run button, reads the proof file, and stores a screenshot. Ctrl+F5 still does not open the Run window in the latest real run. | [RabbitHole Run-window hook #148](https://github.com/rysweet/RabbitHole/pull/148), [eatme Run-window consumer #83](https://github.com/rysweet/eatme/pull/83), [Run-window toolbar proof journal](docs/atlas/journal/0084-run-window-toolbar-proof.md) | Replace the coordinate click with a stronger UI affordance or accessibility path; prove desktop world execution and visible rendering after the Run frame opens. |
